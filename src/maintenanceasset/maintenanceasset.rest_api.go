...
// RESTAPIHandler provides a convenient interface for MaintenanceAsset REST API handler.
type RESTAPIHandler struct {
	UseCase UseCaseHandler
}

// injectDeps inject the dependencies of the MaintenanceAsset REST API handler.
...
// GetByID is the REST API handler for `GET /api/maintenance_assets/{id}`.
...
// Get is the REST API handler for `GET /api/maintenance_assets`.
...
// Create is the REST API handler for `POST /api/maintenance_assets`.
func (r *RESTAPIHandler) Create(c *fiber.Ctx) error {
	...
	res, err := r.UseCase.GetByID(r.UseCase.ID.String)
	if err != nil {
		return app.Error().Handler(c, err)
	}
	resp := app.ListSingleModel{}
...
// UpdateByID is the REST API handler for `PUT /api/maintenance_assets/{id}`.
...
// PartiallyUpdateByID is the REST API handler for `PATCH /api/maintenance_assets/{id}`.
...
// DeleteByID is the REST API handler for `DELETE /api/maintenance_assets/{id}`.
