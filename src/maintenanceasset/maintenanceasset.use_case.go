...
// UseCaseHandler provides a convenient interface for MaintenanceAsset use case, use UseCase to access UseCaseHandler.
...
// GetByID returns the MaintenanceAsset data for the specified ID.
func (u UseCaseHandler) GetByID(id string) (MaintenanceAsset, error) {
	res := MaintenanceAsset{}

	// check permission
	err := u.Ctx.ValidatePermission("maintenance_assets.detail")
	if err != nil {
		return res, err
	}
...
// Get returns the list of MaintenanceAsset data.
func (u UseCaseHandler) Get() (app.ListModel, error) {
	res := app.ListModel{}

	// check permission
	err := u.Ctx.ValidatePermission("maintenance_assets.list")
	if err != nil {
		return res, err
	}
	// get from cache and return if exists
	...
	res.Results.PageContext.Count,
		...
		err = app.Query().PaginationInfo(tx, &MaintenanceAsset{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
	...
	// find data
	data, err := app.Query().Find(tx, &MaintenanceAsset{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
// Create creates a new data MaintenanceAsset with specified parameters.
func (u *UseCaseHandler) Create(p *ParamCreate) error {

	// check permission
	err := u.Ctx.ValidatePermission("maintenance_assets.create")
	if err != nil {
		return err
	}
	...
	err = u.setDefaultValue(MaintenanceAsset{})
	if err != nil {
		return err
	}
...
// UpdateByID updates the MaintenanceAsset data for the specified ID with specified parameters.
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {

	// check permission
	err := u.Ctx.ValidatePermission("maintenance_assets.edit")
	if err != nil {
		return err
	}
...
// PartiallyUpdateByID updates the MaintenanceAsset data for the specified ID with specified parameters.
...
// DeleteByID deletes the MaintenanceAsset data for the specified ID.
func (u UseCaseHandler) DeleteByID(id string, p *ParamDelete) error {

	// check permission
	err := u.Ctx.ValidatePermission("maintenance_assets.delete")
	if err != nil {
		return err
	}
...
// setDefaultValue set default value of undefined field when create or update MaintenanceAsset data.
func (u *UseCaseHandler) setDefaultValue(old MaintenanceAsset) error {
	if !old.ID.Valid {
