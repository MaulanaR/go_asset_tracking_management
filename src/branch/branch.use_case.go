...
// UseCaseHandler provides a convenient interface for Branch use case, use UseCase to access UseCaseHandler.
...
// GetByID returns the Branch data for the specified ID.
func (u UseCaseHandler) GetByID(id string) (Branch, error) {
	res := Branch{}

	// check permission
	err := u.Ctx.ValidatePermission("branches.detail")
	if err != nil {
		return res, err
	}
...
// Get returns the list of Branch data.
func (u UseCaseHandler) Get() (app.ListModel, error) {
	res := app.ListModel{}

	// check permission
	err := u.Ctx.ValidatePermission("branches.list")
	if err != nil {
		return res, err
	}
	// get from cache and return if exists
	...
	res.Results.PageContext.Count,
		...
		err = app.Query().PaginationInfo(tx, &Branch{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
	...
	// find data
	data, err := app.Query().Find(tx, &Branch{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
// Create creates a new data Branch with specified parameters.
func (u UseCaseHandler) Create(p *ParamCreate) error {

	// check permission
	err := u.Ctx.ValidatePermission("branches.create")
	if err != nil {
		return err
	}
...
// UpdateByID updates the Branch data for the specified ID with specified parameters.
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {

	// check permission
	err := u.Ctx.ValidatePermission("branches.edit")
	if err != nil {
		return err
	}
...
// PartiallyUpdateByID updates the Branch data for the specified ID with specified parameters.
...
// DeleteByID deletes the Branch data for the specified ID.
func (u UseCaseHandler) DeleteByID(id string, p *ParamDelete) error {

	// check permission
	err := u.Ctx.ValidatePermission("branches.delete")
	if err != nil {
		return err
	}
...
// setDefaultValue set default value of undefined field when create or update Branch data.
func (u *UseCaseHandler) setDefaultValue(old Branch) error {
