...
// RESTAPIHandler provides a convenient interface for Branch REST API handler.
type RESTAPIHandler struct {
	UseCase UseCaseHandler
}

// injectDeps inject the dependencies of the Branch REST API handler.
...
// GetByID is the REST API handler for `GET /api/branches/{id}`.
...
// Get is the REST API handler for `GET /api/branches`.
...
// Create is the REST API handler for `POST /api/branches`.
...
// UpdateByID is the REST API handler for `PUT /api/branches/{id}`.
...
// PartiallyUpdateByID is the REST API handler for `PATCH /api/branches/{id}`.
...
// DeleteByID is the REST API handler for `DELETE /api/branches/{id}`.
