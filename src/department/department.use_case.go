...
// UseCaseHandler provides a convenient interface for Department use case, use UseCase to access UseCaseHandler.
...
// GetByID returns the Department data for the specified ID.
func (u UseCaseHandler) GetByID(id string) (Department, error) {
	res := Department{}

	// check permission
	err := u.Ctx.ValidatePermission("departments.detail")
	if err != nil {
		return res, err
	}
...
// Get returns the list of Department data.
func (u UseCaseHandler) Get() (app.ListModel, error) {
	res := app.ListModel{}

	// check permission
	err := u.Ctx.ValidatePermission("departments.list")
	if err != nil {
		return res, err
	}
	// get from cache and return if exists
	...
	res.Results.PageContext.Count,
		...
		err = app.Query().PaginationInfo(tx, &Department{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
	...
	// find data
	data, err := app.Query().Find(tx, &Department{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
func (u UseCaseHandler) Create(p *ParamCreate) error {
	...
	err = u.setDefaultValue(Department{})
	if err != nil {
		return err
	}
...
// UpdateByID updates the Department data for the specified ID with specified parameters.
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {

	// check permission
	err := u.Ctx.ValidatePermission("departments.edit")
	if err != nil {
		return err
	}
...
// PartiallyUpdateByID updates the Department data for the specified ID with specified parameters.
...
// DeleteByID deletes the Department data for the specified ID.
func (u UseCaseHandler) DeleteByID(id string, p *ParamDelete) error {

	// check permission
	err := u.Ctx.ValidatePermission("departments.delete")
	if err != nil {
		return err
	}
...
// setDefaultValue set default value of undefined field when create or update Department data.
func (u *UseCaseHandler) setDefaultValue(old Department) error {
