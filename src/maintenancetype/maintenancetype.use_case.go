...
// UseCaseHandler provides a convenient interface for MaintenanceType use case, use UseCase to access UseCaseHandler.
...
// GetByID returns the MaintenanceType data for the specified ID.
func (u UseCaseHandler) GetByID(id string) (MaintenanceType, error) {
	res := MaintenanceType{}

	// check permission
	err := u.Ctx.ValidatePermission("maintenance_types.detail")
	if err != nil {
		return res, err
	}
...
// Get returns the list of MaintenanceType data.
func (u UseCaseHandler) Get() (app.ListModel, error) {
	res := app.ListModel{}

	// check permission
	err := u.Ctx.ValidatePermission("maintenance_types.list")
	if err != nil {
		return res, err
	}
	// get from cache and return if exists
	...
	res.Results.PageContext.Count,
		...
		err = app.Query().PaginationInfo(tx, &MaintenanceType{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
	...
	// find data
	data, err := app.Query().Find(tx, &MaintenanceType{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
// Create creates a new data MaintenanceType with specified parameters.
func (u *UseCaseHandler) Create(p *ParamCreate) error {

	// check permission
	err := u.Ctx.ValidatePermission("maintenance_types.create")
	if err != nil {
		return err
	}
	...
	err = u.setDefaultValue(MaintenanceType{})
	if err != nil {
		return err
	}
...
// UpdateByID updates the MaintenanceType data for the specified ID with specified parameters.
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {

	// check permission
	err := u.Ctx.ValidatePermission("maintenance_types.edit")
	if err != nil {
		return err
	}
...
// PartiallyUpdateByID updates the MaintenanceType data for the specified ID with specified parameters.
...
// DeleteByID deletes the MaintenanceType data for the specified ID.
func (u UseCaseHandler) DeleteByID(id string, p *ParamDelete) error {

	// check permission
	err := u.Ctx.ValidatePermission("maintenance_types.delete")
	if err != nil {
		return err
	}
...
// setDefaultValue set default value of undefined field when create or update MaintenanceType data.
func (u *UseCaseHandler) setDefaultValue(old MaintenanceType) error {
