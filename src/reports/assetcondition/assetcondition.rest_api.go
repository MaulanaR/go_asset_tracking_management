...
// RESTAPIHandler provides a convenient interface for AssetCondition REST API handler.
type RESTAPIHandler struct {
	UseCase UseCaseHandler
}

// injectDeps inject the dependencies of the AssetCondition REST API handler.
...
// Get is the REST API handler for `GET /api/asset_conditions`.
func (r *RESTAPIHandler) Get(c *fiber.Ctx) error {
	...
	return c.Render("report_templates/asset_conditions", res)
}
