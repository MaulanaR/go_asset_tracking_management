...
// Base is common detail of distribution_assets_per_departments open api document component.
func (o *OpenAPIOperation) Base() {
	o.Tags = []string{"DistributionAssetsPerDepartment"}
	o.HeaderParams = []map[string]any{{"$ref": "#/components/parameters/headerParam.Accept-Language"}}
	o.Responses = map[string]map[string]any{
		"200": {
			"description": "Success",
			"content":     map[string]any{"application/json": &DistributionAssetsPerDepartment{}}, // will auto create schema $ref: '#/components/schemas/DistributionAssetsPerDepartment' if not exists
		},
...
// Get is detail of `GET /api/v3/distribution_assets_per_departments` open api document component.
